...
const Social = ({ type, link, text, ...props }: SocialProps) => {
    ...
    return (
        <div className="flex gap-3 ml-3" {...props}>
            <svg className="h-6 w-6 viewBox[0 0 24 24] fill-slate-200" xmlns="http://www.w3.org/2000/svg">
                <path d={properties().svgPath} />
            </svg>
            <h4>
                {properties().title}: {link ? <a href={link}>{text}</a> : text}
            </h4>
        </div>
    );
};
