...
const Social = ({ type, link, text }: SocialProps) => {
    const svgPaths: Record<SocialProps['type'], string> = {
    ...
    const titles: Record<SocialProps['type'], string> = {
        github: 'GitHub',
        linkedin: 'LinkedIn',
        email: 'Email',
    };
    ...
    return (
        <a
            ...
            className="flex items-center gap-3 text-slate-400 hover:text-slate-200 transition-colors group w-fit"
        >
            <svg
                className="h-5 w-5 fill-current flex-shrink-0"
                viewBox="0 0 24 24"
